...
static struct {
    ...
    EGLStreamKHR stream;
    struct wl_resource *stream_buffer_resource;
} egl_data;
...
on_surface_frame (void *data, struct wl_callback *callback, uint32_t time)
{
    ...
    EGLBoolean did_release = egl_data.stream_consumer_release (egl_data.display, egl_data.stream);
...
request_frame (void)
{
    ...
    if (0 && wl_data.presentation != NULL) {
...
{
    g_source_set_ready_time (win_data.repaint_source, 0);
}

static void
on_export_eglstream_producer_resource (void *data, struct wl_resource *buffer_resource)
{
    if (!egl_data.create_stream_attrib) {
        egl_data.create_stream_attrib = (PFNEGLCREATESTREAMATTRIBKHRPROC)
                                         eglGetProcAddress ("eglCreateStreamAttribNV");
    ...
    egl_data.stream = egl_data.create_stream_attrib (egl_data.display, attrib_list);
    egl_data.stream_buffer_resource = buffer_resource;

    if (!egl_data.stream_consumer_gl_texture_external) {
        egl_data.stream_consumer_gl_texture_external = (PFNEGLSTREAMCONSUMERGLTEXTUREEXTERNALKHRPROC)
                                                        eglGetProcAddress ("eglStreamConsumerGLTextureExternalKHR");
        g_assert (egl_data.stream_consumer_gl_texture_external);
    }

    eglMakeCurrent (egl_data.display, win_data.egl_surface, win_data.egl_surface, egl_data.context);

    glActiveTexture (GL_TEXTURE0);
    glBindTexture (GL_TEXTURE_EXTERNAL_OES, gl_data.texture);

    EGLBoolean result = egl_data.stream_consumer_gl_texture_external (egl_data.display, egl_data.stream);

    glBindTexture (GL_TEXTURE_EXTERNAL_OES, 0);
}

static gboolean
repaint_source_dispatch (GSource * source, GSourceFunc callback, gpointer user_data)
{
    g_source_set_ready_time (source, -1);

    if (!egl_data.stream_consumer_acquire) {
        egl_data.stream_consumer_acquire = (PFNEGLSTREAMCONSUMERACQUIREKHRPROC)
                                            eglGetProcAddress ("eglStreamConsumerAcquireKHR");
        g_assert (egl_data.stream_consumer_acquire);
    }

    if (!egl_data.stream_consumer_release) {
        egl_data.stream_consumer_release = (PFNEGLSTREAMCONSUMERRELEASEKHRPROC)
                                            eglGetProcAddress ("eglStreamConsumerReleaseKHR");
        g_assert (egl_data.stream_consumer_release);
    }
    ...
    EGLBoolean did_acquire = egl_data.stream_consumer_acquire (egl_data.display, egl_data.stream);
...
create_window (GError **error)
{
    ...
    win_data.egl_window = wl_egl_window_create (win_data.wl_surface, DEFAULT_WIDTH, DEFAULT_HEIGHT);
    if (win_data.egl_window == NULL) {
        ERR_EGL (error, "Could not create EGL window");
        return FALSE;
    }

    win_data.egl_surface = eglCreateWindowSurface (egl_data.display, egl_data.egl_config, win_data.egl_window, NULL);
    if (win_data.egl_surface == EGL_NO_SURFACE) {
        ERR_EGL (error, "Could not create EGL surface");
        return FALSE;
    }

    static GSourceFuncs source_funcs = {
            NULL, NULL, repaint_source_dispatch,
            NULL, NULL, NULL,
        };
    win_data.repaint_source = g_source_new (&source_funcs, sizeof (GSource));
    g_source_set_ready_time (win_data.repaint_source, -1);
    g_source_set_priority (win_data.repaint_source, G_PRIORITY_DEFAULT);
    g_source_attach (win_data.repaint_source, g_main_context_get_thread_default ());
                               ...
                               GError       **error)
{
    static struct wpe_view_backend_exportable_fdo_client exportable_client = {
        ...
        .export_eglstream_producer_resource = on_export_eglstream_producer_resource,
    };
